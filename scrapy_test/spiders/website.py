...
class WebsiteSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        #phone_numbers = re.findall(r'\b(?:\+\d{2}\s?)?(?:(?:\d{2}\s\d{2}\s\d{9})|(?:\d{4}\s\d{3}\s\d{4})|(?:\d{4}\s\d{4}))\b', text)
        
        #phone_numbers = re.findall(r'\b(?:\+\d{2}\s?)?(?:(?:\d{2}\s\d{2}\s\d{9})|(?:\d{4}\s\d{3}\s\d{4})|(?:\d{4}\s\d{4})|(?:0800\s\d{3}\s\d{4})|(?:\(?\d{3}\)?\s?\d{4}\s?\d{4})|(?:\(\d{3}\)\s?\d{3}-\d{4})|(?:\d{3}-\d{4})|(?:\(\d{3}\)\s\d{3}-\d{4}))\b', text)
